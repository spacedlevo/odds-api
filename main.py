...
def write_to_csv(jsondata):
    ...
    with open('uefa_odds.csv', 'w', newline='') as csvfile:
        ...
        for match in jsondata:
            row = [match['id'], f"{match['home_team']} vs {match['away_team']}", match['home_team'], match['away_team']]
            ...
            home_win_mean = round(statistics.mean(home_win_odds),2) if home_win_odds else None
            draw_mean = round(statistics.mean(draw_odds),2) if draw_odds else None
            away_win_mean = round(statistics.mean(away_win_odds),2) if away_win_odds else None
